...
int main(int argc, char **argv)
{
	...
	// table that will contain all times for which the orders
	// were cold
	...
	// sums to calculate average time it took for the
	// for the order to be completed and for which the
	// order was cold
	int sum = 0;
	int sum2 = 0;
	...
	// calculating max time of an order and max time
	// of an order being cold
	for (int i = 1; i <= Ncust; i++)
	{
		...
		if (Cold_times[i] > max2)
		{
			max2 = Cold_times[i];
		}
	}

	// print messages for max and average time of order
	// completion and order being cold
...
void *order(void *x)
{
	...
	// now the order is ready to enter the oven, so we lock it with
	// the appropriate mutex and check if there are any available ovens
	...
	// The cook has placed the order in the oven so he is
	// so he is now available
	rc = pthread_mutex_lock(&cook_lock);
	rc_check(rc);
	Ncook++;

	// signal the threads that were waiting for a cook
	// to be available
	...
	// while there are no deliverers available the thread
	// waits
	while (Ndeliverer == 0)
	{
	...
	// signal the threads that were waiting to enter to
	// an oven
	...
	* in F_times table and Cold_times table respectively
	*/
	...
	// the deliverer has returned and is now available
	// so we lock the appropriate mutex
	rc = pthread_mutex_lock(&deliverer_lock);
	Ndeliverer++;

	// signal the threads that are sleeping
